...
export default function useMPs({ party, is_minister, search, sort, direction, page, limit }) {
  const [state, setState] = useState({ loading: true, data: null, error: null, summary: null });

  useEffect(() => {
    setState((current) => ({ ...current, loading: true }));
    fetchMPs({ party, is_minister, search: search || undefined, sort, direction, page, limit })
  ...
  }, [party, is_minister, search, sort, direction, page, limit]);
