...
**`total_score`** (on `MPSummary`) is the mean of an MP's available 0–100 metrics — attendance/questions/debates/pmb scores **plus `clean_record_score` plus `mplads_score`**. `mplads_score` uses a composite formula: raw weighted ratio of four sub-metrics (mobilization 30%, completion 30%, utilization 25%, payment efficiency 15%) → single percentile rank. "Completion Rate %" in the CSV = `completed_works / (completed_works + recommended_works)` (not completed/recommended). Because `clean_record_score` is derived in Python (not a SQL column), sorting by `total_score` can't be done in SQL: the `sort=total_score` branch in `list_mps` fetches all matching rows, builds summaries, then sorts & paginates in-process (fine at ~550 rows). MPs with no scored metrics sort last in both directions. The frontend card donut reuses `mp.total_score` when present, else recomputes client-side (e.g. the leaderboard endpoint doesn't supply it).
